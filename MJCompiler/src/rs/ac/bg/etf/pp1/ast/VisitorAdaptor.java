// generated with ast extension for cup
// version 0.8
// 20/7/2022 8:51:47


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Mulop Mulop) { }
    public void visit(ExprWrapper ExprWrapper) { }
    public void visit(ArrayOptional ArrayOptional) { }
    public void visit(Constants Constants) { }
    public void visit(Variable Variable) { }
    public void visit(StatementList StatementList) { }
    public void visit(ActParams ActParams) { }
    public void visit(PrintOptional PrintOptional) { }
    public void visit(Addop Addop) { }
    public void visit(ReturnOptional ReturnOptional) { }
    public void visit(DeclList DeclList) { }
    public void visit(Term Term) { }
    public void visit(MinusOptional MinusOptional) { }
    public void visit(FormParams FormParams) { }
    public void visit(CallOptional CallOptional) { }
    public void visit(RightSide RightSide) { }
    public void visit(BaseFactor BaseFactor) { }
    public void visit(Expr Expr) { }
    public void visit(Variables Variables) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(FormParamList FormParamList) { }
    public void visit(Statement Statement) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(Type Type) { }
    public void visit(Declaration Declaration) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(ConstantValues ConstantValues) { }
    public void visit(FormParam FormParam) { }
    public void visit(ModOp ModOp) { visit(); }
    public void visit(DivOp DivOp) { visit(); }
    public void visit(MultOp MultOp) { visit(); }
    public void visit(MinusOp MinusOp) { visit(); }
    public void visit(PlusOp PlusOp) { visit(); }
    public void visit(NoActParam NoActParam) { visit(); }
    public void visit(HaveActParams HaveActParams) { visit(); }
    public void visit(NoCallOpt NoCallOpt) { visit(); }
    public void visit(CallOpt CallOpt) { visit(); }
    public void visit(BaseParens BaseParens) { visit(); }
    public void visit(BaseNew BaseNew) { visit(); }
    public void visit(BaseConst BaseConst) { visit(); }
    public void visit(BaseCall BaseCall) { visit(); }
    public void visit(NoMinusOpt NoMinusOpt) { visit(); }
    public void visit(MinusOpt MinusOpt) { visit(); }
    public void visit(Factor Factor) { visit(); }
    public void visit(SingleFactor SingleFactor) { visit(); }
    public void visit(MultipleFactors MultipleFactors) { visit(); }
    public void visit(SingleTerm SingleTerm) { visit(); }
    public void visit(MultipleTerms MultipleTerms) { visit(); }
    public void visit(NotZeroExpr NotZeroExpr) { visit(); }
    public void visit(NormalExpr NormalExpr) { visit(); }
    public void visit(MinusMinus MinusMinus) { visit(); }
    public void visit(PlusPlus PlusPlus) { visit(); }
    public void visit(EqualAssign EqualAssign) { visit(); }
    public void visit(NoArrayOpt NoArrayOpt) { visit(); }
    public void visit(ArrayOpt ArrayOpt) { visit(); }
    public void visit(DesignatorName DesignatorName) { visit(); }
    public void visit(Designator Designator) { visit(); }
    public void visit(DesignatorError DesignatorError) { visit(); }
    public void visit(DesignatorStatementV DesignatorStatementV) { visit(); }
    public void visit(NoReturnOpt NoReturnOpt) { visit(); }
    public void visit(ReturnOpt ReturnOpt) { visit(); }
    public void visit(NoPrintOpt NoPrintOpt) { visit(); }
    public void visit(PrintOpt PrintOpt) { visit(); }
    public void visit(StBlock StBlock) { visit(); }
    public void visit(ReturnSt ReturnSt) { visit(); }
    public void visit(PrintSt PrintSt) { visit(); }
    public void visit(ReadSt ReadSt) { visit(); }
    public void visit(DesignatorSt DesignatorSt) { visit(); }
    public void visit(NoStatements NoStatements) { visit(); }
    public void visit(HaveStatements HaveStatements) { visit(); }
    public void visit(FormParamArray FormParamArray) { visit(); }
    public void visit(FormParamNormal FormParamNormal) { visit(); }
    public void visit(SingleFormParam SingleFormParam) { visit(); }
    public void visit(MultipleFormParams MultipleFormParams) { visit(); }
    public void visit(NoFormParams NoFormParams) { visit(); }
    public void visit(HaveFormParams HaveFormParams) { visit(); }
    public void visit(MethodName MethodName) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(NoMetDecl NoMetDecl) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(ConstChar ConstChar) { visit(); }
    public void visit(ConstBool ConstBool) { visit(); }
    public void visit(ConstInt ConstInt) { visit(); }
    public void visit(Constant Constant) { visit(); }
    public void visit(SingleConstant SingleConstant) { visit(); }
    public void visit(MultipleConstants MultipleConstants) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(VariableArray VariableArray) { visit(); }
    public void visit(VariableNormal VariableNormal) { visit(); }
    public void visit(SingleVariable SingleVariable) { visit(); }
    public void visit(VariablesList VariablesList) { visit(); }
    public void visit(StandardType StandardType) { visit(); }
    public void visit(VoidType VoidType) { visit(); }
    public void visit(StandardTypes StandardTypes) { visit(); }
    public void visit(VarDeclV VarDeclV) { visit(); }
    public void visit(DeclarationErr DeclarationErr) { visit(); }
    public void visit(DeclarationConst DeclarationConst) { visit(); }
    public void visit(DeclarationVar DeclarationVar) { visit(); }
    public void visit(NoDeclList NoDeclList) { visit(); }
    public void visit(DeclsList DeclsList) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
